...
class LineChartSample6 extends StatelessWidget {
  ...
  final spots = [
    for (int i = 0; i < classComparison.length; i++)
      FlSpot(
          i + 1, double.parse(classComparison[i]['classAverage'].toString())),
  ];

  final spots2 = [
    for (int i = 0; i < classComparison.length; i++)
      FlSpot(i + 1,
          double.parse(classComparison[i]['studentProgress'].toString())),
  ];
  ...
  Widget topTitleWidgets(double value, TitleMeta meta) {
    final titles = classComparison.map((e) => e['subject']).toList();
    ...
    return SideTitleWidget(
      ...
      child: Text(titles[value.toInt() - 1].toString(), style: style),
    );
  }
